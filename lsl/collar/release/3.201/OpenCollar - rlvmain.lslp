...
CheckVersion()
{
    ...
    if (verbose)
    {
        llOwnerSay("Attempting to enable Restrained Life Viewer functions.  " + rlvString+ " or higher is required for all features to work.");        
    }
    //open listener
...
sendCommand(string cmd)
{
    ...
    if (rlvnotify)
    {
        llOwnerSay("Sent RLV Command: " + cmd);
    }
...
state checked
{
    ...
    link_message(integer sender, integer num, string str, key id)
    {
        ...
        {   //if RLV is off, don't even respond to RLV submenu events
            ...
            else if (num == COMMAND_RLV_RELAY && timertype=="pong" && str=="ping,"+(string)llGetOwner()+",!pong")
            {
            ...
            else if (num >= COMMAND_OWNER && num <= COMMAND_WEARER)
            {
                ...
                else if (str == "rlvoff")
                {
                    ...
                    {
                        llInstantMessage(id, "Sorry, only owner may disable Restrained Life functions");
                    }

...
key cardid;//used to detect whether coupleanims card has changed
float range = 10.0;//only scan within this range for anim partners
float tau = 0.5; //how hard to push sub toward 
...
integer ValidLine(list params)
{
    ...
    {
        llOwnerSay(CARD + " line " + (string)line + ": animation '" + llList2String(params, 1) + "' is not present.  Skipping.");
        return FALSE;
    }
    else if (!AnimExists(llList2String(params, 2)))
    {
        llOwnerSay(CARD + " line " + (string)line + ": animation '" + llList2String(params, 2) + "' is not present.  Skipping.");        
        return FALSE;
    }
    else
    {
...
{    
        ...
        if (num >= COMMAND_OWNER && num <= COMMAND_WEARER)
        {
            ...
            if (tmpindex != -1)
            {
                ...
                else
                {
                    ...
                    if (id == llGetOwner())
                    {
                        llMessageLinked(LINK_THIS, POPUP_HELP, "Error: you didn't give the name of the person you want to animate.  To " + cmd + " Nandana Singh, for example, you could say /_CHANNEL__PREFIX" + cmd + " nan", llGetOwner());
                    }
                    else
                    {
                        partner = cmdgiver;
    ...
    at_target(integer tnum, vector targetpos, vector ourpos)
    {
        if (!arrived)
        {
            ...
            if (text != "")
            {
                text = str_replace(text, "_SELF_", FirstName(llKey2Name(llGetOwner())));
    ...
    changed(integer change)
    {
        if (change & CHANGED_INVENTORY)
        {
            if (llGetInventoryKey(CARD) != cardid)
            {

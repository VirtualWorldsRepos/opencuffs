...
integer remoteon = TRUE;
float timeout = 30.0;
string queueurl = "http://collardata.appspot.com/remote/";
key queueid;

list defaults;
list queryqueue;//requests are stuck here until we're done reading the notecard and web settings
...
HandleRequest(integer num, string str, key id)
{
    ...
    else if (num >= COMMAND_OWNER && num <= COMMAND_WEARER)
    {
        ...
        else if (str == "remoteon")
        {
            ...
            HandleRequest(HTTPDB_SAVE, "remoteon=1", NULL_KEY);
        }
        else if (str == "remoteoff")
        {
            ...
            else
            {
                ...
                HandleRequest(HTTPDB_SAVE, "remoteon=0", NULL_KEY);                 
            }                   
        }
    }
    else if (num == SUBMENU)
...
default
{
    state_entry()
    {       
        cacheid = llHTTPRequest(HTTPDB + ALLTOKEN, [HTTP_METHOD, "GET"], "");
        ...
        {
            //default settings card not found, go to ready state
            gotdefaults = TRUE;
        }
    }
...
state ready
{
    ...
    http_response( key id, integer status, list meta, string body ) 
    {
        ...
        if ( index != -1 ) 
        {
            ...
            if (status == 200)
            {
                //remove the dbprefix prefix
                //token = llGetSubString(token, llStringLength(dbprefix), -1);
                string out = token + "=" + body;
                //only send response if not blank, 

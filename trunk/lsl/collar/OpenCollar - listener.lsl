        ...
        if (channel == objectchannel)
        {
            ...
            else
            {
                ...
                if (llGetListLength(relaybits) == 3 && llList2String(relaybits, 1) == (string)llGetOwner())
                {
                    ...
                    llMessageLinked(LINK_THIS, COMMAND_RLV_RELAY, message, id);
                }
            }
        }
        ...
        if (num == COMMAND_OWNER)
        {
            ...
            else if (command == "channel")
            {
                ...
                if (newchannel > 0)
                {
                    ...
                    llMessageLinked(LINK_THIS, HTTPDB_SAVE, "channel=" + (string)listenchannel, NULL_KEY);                         
                }
                else
                {
                    //they left the param blank or tried to use 0
                    Popup(id, "Error: 'channel' must be set to a whole number greater than 0.");
                }
            }

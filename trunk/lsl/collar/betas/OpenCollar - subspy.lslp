...
key owner = NULL_KEY;
integer ownerOnline;
integer lastOwnerOnlineCheck;
key ownerOnlineDSEvent;
string subName;
integer menuchannel;
integer menuhandle;
string location;
list avsNearby;
integer sensorRange = 8;
integer sensorRepeat = 300;
...
DialogSpy(key id)
{
 ...
 //taking this out for now as the cache/buffer feature seems unreliable
    //text += "cacheOn/Off decides if you get spy information cached and sent as bundle in an email while you are offline or same as when you are online, every indo right away.\n";
...
{
    currentmenu = "spy";
    string text = "Only the Owner can set and see spy options.";
    text += "This menu will time out in " + (string)timeout + " seconds.";
    list buttons = [UPMENU];
    buttons = RestackMenu(buttons);
    menuhandle = llListen(menuchannel, "", id, "");
...
SendIM(key id, string str)
{
    ...
    {
        if (id != owner) 
        {// if it's not to the owner, just send it
            llInstantMessage(id, str);
        } 
        else 
        {
            integer now = llGetUnixTime();
            if (now > lastOwnerOnlineCheck+30) 
            {// check again to see if the owner is online
                ownerOnlineDSEvent = llRequestAgentData(owner,DATA_ONLINE);
                lastOwnerOnlineCheck = now;
            }
            // we probably don't have the answer to the online check yet, but we will by next time and it probably didn't change
            if (ownerOnline || !enabled("cache")) 
            {
                // send any leftover messages
                if (llStringLength(accumulatedMessage) > 0) 
                {
                    llInstantMessage(id, accumulatedMessage);
                    accumulatedMessage = "";
                }
                // and send the current message immediately
                llInstantMessage(id, str);
            } 
            else 
            {// the owner is offline, so buffer the message until we reach 1K
                if (llStringLength(accumulatedMessage)+llStringLength(str) > 1000) 
                {// we need to send accumulatedMessage now
                    llInstantMessage(id, accumulatedMessage);
                    accumulatedMessage = "";
                }
                accumulatedMessage = accumulatedMessage + GetTimestamp() + ": " + str + "\n";
            }
        }
    }
}
...
SendNotify(string newlocation)
{
    if (ownerOnline || !enabled("cache")) 
    {
        SendIM(owner,subName + " teleported from " + location + " to " +  newlocation + ".");
    } 
    else 
    {
        SendIM(owner,subName + " teleported to " +  newlocation + ".");
    }
    location = newlocation;
}

SendLogin(string location) 
{
    SendIM(owner,subName+" logged in at "+location);
}
...
SetSettings()
{
    ...
    for(i = 1; i < listlength; i += 2)
    {
        ...
        if(option == "listen")
        {
            if(value == "on")
            {
                listenhandle = llListen(0, subName, wearer, "");
            }
            else if(value == "off")
            {
        ...
        {
            sensorRange = (integer)value;
        }
        else if(option == "minutes")
        {
            sensorRepeat = (integer)value;
        }
    }
...
default
{
    ...
    listen(integer channel, string name, key id, string message)
    {
        ...
        else if(channel == menuchannel)
        {
            ...
            else if(currentmenu == "radarsettings")
            {
                ...
                integer value = (integer)llList2String(temp,0);
    ...
    {  //only the primary owner can use this !!
        if (auth == COMMAND_OWNER)
        {
            ...
            else if (str == "cache on")
            {
                SaveSettings(str, id);
                SendIM(id, "You cache reports while you are offline to be send in collected form now to send less messages to you.");
            }
            else if (str == "cache off")
            {
                SaveSettings(str, id);
                SendIM(id, "You get now every report sent directly if you are online or offline.");
            }
        }
        else if (auth == HTTPDB_SAVE)
        {
        ...
        {
            list cmds = ["trace on","trace off", "radar on", "radar off", "listen on", "listen off", "cache on", "cache off"];
    ...
    {
        if (enabled("trace") && llGetAttached()) 
        {// log the wearer's login
            SendLogin(llGetRegionName());
        }
    }
    ...
    {
        if (requested == ownerOnlineDSEvent) 
        {
            ownerOnline = (integer)data;
            ownerOnlineDSEvent = NULL_KEY;
        }
    }
}

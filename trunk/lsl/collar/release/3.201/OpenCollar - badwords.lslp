...
{    
    ...
    listen(integer channel, string name, key id, string message)
    {
        if(channel == menuchannel)
        {
            ...
            {
                DialogBadwords(id);
                llInstantMessage(id, "The penance phrase to release the sub from the punishment anim is:\n" + penance);
            }
        ...
        {      
            ...
            {   //start anim
                ...
                llWhisper(0, llList2String(llParseString2List(llKey2Name(llGetOwner()), [" "], []), 0) + " has said a bad word and is being punished.");
                hasSworn = TRUE;        
            }      
        }  
    }
    ...
    link_message(integer sender, integer num, string str, key id)
    {
        ...
        else if(num > COMMAND_OWNER && num <= COMMAND_EVERYONE)
        {
            ...
            if(command == "badwords")
            {
                llInstantMessage(id, "Sorry, only the owner can toggle badwords.");
            }
        }

...
from dbdefinitions import FreebieItem, FreebieDelivery
...
class Deliver(webapp.RequestHandler):
    def post(self):
        ...
        if not lindenip.inrange(os.environ['REMOTE_ADDR']):
            self.error(403)
        elif not distributors.vendor_authorized(self.request.headers['X-SecondLife-Owner-Key']):
            self.error(403)
        else:
...
class AddVend(webapp.RequestHandler):
    def post(self):
        if not lindenip.inrange(os.environ['REMOTE_ADDR']):
            self.error(403)
            ...
            for line in lines:
                params[line.split('=')[0]] = line.split('=')[1]
            distributors.vendor_add(params['key'], params['name'])
            self.response.out.write('added vendor %s' % params['name'])
